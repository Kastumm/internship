...
//mocking Data
function mockingData(array) {
    array.push(new Person("Emil", 24));
    array.push(new Person("Eliodor", 25));
    array.push(new Person("John Doe", 36));
}
function writeFileAsync(arrayOfPersons) {
    return new Promise(function (resolve, reject) {
        var data = JSON.stringify(arrayOfPersons);
        fs.writeFile("persons.json", data, function (error) {
            if (error) {
...
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var personsArray, message, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    personsArray = [];
                    mockingData(personsArray);
                    return [4 /*yield*/, writeFileAsync(personsArray)];
                case 1:
                    message = _a.sent();
                    console.log(message);
                    data = readFile('persons.json');
                    console.log(data);
